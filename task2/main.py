...
def generate_intervention(fake, id, start_date=None, end_date=None):
    ...
    intervention.append(fake.date_between_dates(date_start=start_date, date_end=end_date).strftime('%d.%m.%Y'))
...
def generate_policeman_intervention_relationship(start_id=0, second_no_of_interventions = 0, policemen = policemen_filename):
    p_i_relationship_df = pd.DataFrame(columns=['ID_interwencji', 'PESEL_policjanta'])
    stations = pd.read_csv(stations_filename)

    relationship_file_idx = start_id

    for i in range(second_no_of_interventions, NO_OF_INTERVENTIONS + second_no_of_interventions):
