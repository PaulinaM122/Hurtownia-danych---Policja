...
NO_OF_POLICEMEN = 10000*6
NO_OF_CRIMINALS = 10000*6
NO_OF_STATIONS = 1000*6
NO_OF_EQUIPMENT = 5000*6
NO_OF_INTERVENTIONS = 10000*6
...
def generate_policemen(start_id=0):
    fake = Faker('pl_PL')
    ...
    for i in range(start_id, start_id + NO_OF_POLICEMEN):
...
def generate_criminals(start_id=0):
    criminals_df = pd.DataFrame(columns=['PESEL', 'Imię', 'Nazwisko', 'Płeć', 'Adres', 'Czy_zatrzymany'])
    for i in range(start_id, start_id + NO_OF_CRIMINALS):
...
def generate_police_stations(start_id=0):
    ...
    for i in range(start_id, start_id + NO_OF_STATIONS):
...
def generate_equipment(start_id=0):
    fake = Faker()
    equipment_type_provider = DynamicProvider(
        provider_name="equipment_type",
        elements=["bron", "pojazd", "srodki_lacznosci", "inne"]
    )
    ...
    other_police_equipment_provider = DynamicProvider(
        ...
        elements=["Kamizelka kuloodporna", "Kajdanki", "Apteczka taktyczna", "Alkomat AlcoQuant 6020", "Urzadzenie do detekcji narkotykow AlereTM DDS 2", "Latarka taktyczna"]
    )
    fake.add_provider(other_police_equipment_provider)

    equipment_df = pd.DataFrame(columns=['ID', 'Nazwa', 'Numer_seryjny', 'Rodzaj'])
    for i in range(start_id, start_id + NO_OF_EQUIPMENT):
...
def generate_interventions(start_id=0):
    fake = Faker('pl_PL')
    crime_type_provider = DynamicProvider(
        provider_name="crime_type",
        elements=["kradziez", "zniszczenie mienia", "zastraszanie", "rozboj", "przeciwko zyciu czlowieka", "spowodowanie uszczerbku na zdrowiu", "oszustwo", "inne"]
    )
    ...
    for i in range(start_id, start_id + NO_OF_INTERVENTIONS):
...
def generate_intervention(fake, id, start_date=None, end_date=None):
    intervention = []
    intervention.append(id)

    if start_date is None:
        start_date = datetime.date(2023, 10, 1)  # Domyślna data początkowa, jeśli nie podano innej
    if end_date is None:
        end_date = datetime.date(2023, 11, 1)    # Domyślna data końcowa, jeśli nie podano innej

    intervention.append(fake.date_between_dates(date_start=start_date, date_end=end_date))
    intervention.append(fake.address())
    intervention.append(fake.text(max_nb_chars=20))
    intervention.append(fake.crime_type())
    status = random.randint(0, 1)
    if status:
        intervention.append("Zakonczona")
...
def get_policemen_by_station(station_id, policemen):
    policemen = pd.read_csv(policemen)
    return policemen[policemen['Komisariat_FK'] == station_id]


def generate_criminal_intervention_relationship(start_id=0, second_no_of_criminals=0, criminals_f = criminals_filename):
    c_i_relationship_df = pd.DataFrame(columns=['ID_interwencji', 'PESEL_przestępcy'])
    interventions = pd.read_csv(interventions_filename)
    criminals = pd.read_csv(criminals_f)

    relationship_file_idx = start_id

    for i in range(len(interventions)):
        random_criminal_idx = random.randint(second_no_of_criminals, second_no_of_criminals + NO_OF_CRIMINALS - 1)
...
def generate_policeman_intervention_relationship(start_id=0, second_no_of_interwentions = 0, policemen = policemen_filename):
    p_i_relationship_df = pd.DataFrame(columns=['ID_interwencji', 'PESEL_policjanta'])
    stations = pd.read_csv(stations_filename)

    relationship_file_idx = start_id

    for i in range(second_no_of_interwentions, NO_OF_INTERVENTIONS + second_no_of_interwentions):
        random_station_idx = random.randint(0, NO_OF_STATIONS-1)
        policemen_from_station = get_policemen_by_station(random_station_idx, policemen)
        max_policemen_on_intervention = min(stations.loc[random_station_idx]['Zatrudnieni_funkcjonariusze'], 7)
        no_of_policemen_on_intervention = random.randint(1, max_policemen_on_intervention)
...
def generate_equipment_intervention_relationship(start_id=0, equipment_f=equipment_filename):
    interventions = pd.read_csv(interventions_filename)
    equipment = pd.read_csv(equipment_f)
