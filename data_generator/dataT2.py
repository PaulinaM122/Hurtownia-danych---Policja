...
def generate_data_T2():
    ...
    # main.generate_police_stations(start_id=last_station_id + 1) wydaje mi się, że bez sensu jest generować kolejene komisariaty
    main.generate_equipment(start_id=last_equipment_id + 1)
    interventions_T1 = modify_interventions_T1(interventions_T1)
    generate_interventions(fake=Faker('pl_PL'), id=last_intervention_id + 1, start_date=datetime.date(start_date_year, start_date_month, start_date_day), end_date=datetime.date(end_date_year, end_date_month, end_date_day))
